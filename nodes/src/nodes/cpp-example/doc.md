...
One consequence has no workaround: `thread_local` storage cannot cross a shared
library boundary on Windows, so every node carries its own copy of
`ap::async::ThreadApi::m_thisCtx` (`src/threadLocal.cpp`). A node's thread
context is its own, independent of the engine's.

For the same reason, only a host built on the shared engine module can load a
